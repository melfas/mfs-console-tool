...
static void *read_thread(void *param)
{
	...
	// printf("dev->shutdown_thread end\n");
...
/* 	printf("dev->output_endpoint : [%d]\r\n", dev->output_endpoint);
	printf("skipped_report_id : [%d]\r\n", skipped_report_id);
	printf("report_number : [%d]\r\n", report_number);
	for (int j = 0; j < length; j++)
	{
		printf("data[%02d] : 0x%02X  ", j, data[j]);
		if (j > 0 && j % 9 == 0)
			printf("\r\n");
	}
	printf("\r\n");
...
#if 0
	printf("return_data length : %d\r\n", length);
	int i = 0;
	for (i = 0; i < len; i++)
		printf("%02hhx ", data[i]);
	printf("\n");

	printf("return_data rpt->len : %d\r\n", rpt->len);
	for (i = 0; i < len; i++)
		printf("%02hhx ", rpt->data[i]);
	printf("\n");
#endif
...
int HID_API_EXPORT hid_read(hid_device *dev, unsigned char *data, size_t length)
{
	...
	return hid_read_timeout(dev, data, length, 2000);
}
...
void HID_API_EXPORT hid_close(hid_device *dev)
{
	...
	// printf("hid_close : wait for read_thread\n");

	/* Wait for read_thread() to end. */
	pthread_join(dev->thread, NULL);

	// printf("hid_close : read_thread to end\n");
